...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'content-creator', 'guest-contributor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ThinkerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/thinkers` : null),
    () => getThinkers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Thinker',
      path: isTenantUser ? `/thinkers/view/${data?.[0]?.id}` : '/thinkers',
      entity: 'thinker',
      service: AccessServiceEnum.PROJECT,
      icon: FiFeather,
    },
    { name: 'Resource', path: '/resources', entity: 'resource', service: AccessServiceEnum.PROJECT, icon: FiBook },
    {
      name: 'Suggested Edit',
