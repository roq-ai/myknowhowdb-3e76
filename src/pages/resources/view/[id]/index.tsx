...
import { deleteSuggestedEditById } from 'apiSdk/suggested-edits';
...
function ResourceViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ResourceInterface>(
    ...
    () =>
      getResourceById(id, {
        relations: ['thinker', 'user', 'suggested_edit'],
      }),
  );

  const suggested_editHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              {hasAccess('thinker', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/thinkers/view/${data?.thinker?.id}`}>
                      {data?.thinker?.name}
                    </Link>
                  </Text>
                </Flex>
              ...
              {hasAccess('suggested_edit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/suggested-edits/create?resource_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.suggested_edit?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/suggested-edits/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('suggested_edit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/suggested-edits/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('suggested_edit', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    suggested_editHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
